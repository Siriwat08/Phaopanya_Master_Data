...
function cleanupPhase4() {
  ...
  // ★ rev.2: เพิ่ม MAPS_URL (จากแท็บ หรือคำนวณจากพิกัดแถวนั้น) ให้ทีม SCG ตรวจตาได้
  function mapsUrlOfRow(sheetRowNum) {
    var di = sheetRowNum - 2;
    if (di >= 0 && di < n) {
      return clMapsUrl_(vals[di][c.LAT], vals[di][c.LNG]);
    }
    return '';
  }
  var docRows = [];
  ...
  if (tab) {
    for (var i = 0; i < tab.rows.length; i++) {
      var r = tab.rows[i];
      var h = tab.headers;
      var rowNum = Number(r[h.indexOf('ROW')]) || 0;
      var mu = String(r[h.indexOf('MAPS_URL')] || '');
      if (!mu) mu = mapsUrlOfRow(rowNum);
      docRows.push([r[h.indexOf('ROW')], r[h.indexOf('MD_ID')],
        ...
        r[h.indexOf('GEO_LAYER')], r[h.indexOf('KNN_VOTES_W')], mu]);
    }
  } else {
    ...
    for (var i = 0; i < n; i++) {
      ...
      docRows.push([i + 2, clStr_(row[c.MD_ID]), clStr_(row[c.NAME]),
        ...
        clStr_(row[c.AA]), v.vw, clMapsUrl_(row[c.LAT], row[c.LNG])]);
    }
  }
  cleanupReportTab_('P4_ที่อยู่พิมพ์ผิด', ['ROW', 'MD_ID', 'NAME_CLEAN',
    ...
    'ไปรษณีย์', 'GEO_LAYER', 'โหวตพิกัด', 'MAPS_URL'], docRows);

...
/** NFC normalize + trim (เทียบเท่า norm ของ Python) — คง Date และ ★ rev.2 คง Number ไว้
 *  (เดิมแปลง Number เป็น String ทุกค่า → เฟสที่เขียนทั้งคอลัมน์กลับ เช่น U ของเฟส 2
 *   จะเปลี่ยนชนิดข้อมูลทั้งคอลัมน์บน production โดยไม่ตั้งใจ) */
function clNorm_(v) {
...
/** ★ rev.2: ทดสอบว่า cleanThai ที่วางอยู่ในโปรเจกต์เป็นรุ่นแพตช์แล้ว (2 กฎใหม่)
 *  ใช้เป็นทั้งข้อมูลรายงาน (เฟส 1a) และประตูบังคับ (เฟส 1b) */
...
/** ★ rev.2: ลิงก์ Google Maps ของพิกัด (คอลัมน์ MAPS_URL ในแท็บรายงาน)
 *  เขียนเป็นสูตร =HYPERLINK เพื่อคลิกได้บนชีต (mock/Node เก็บเป็นสตริง) */
function clMapsUrl_(lat, lng) {
  var la = clStr_(lat).trim(), lo = clStr_(lng).trim();
  if (la === '' || lo === '') return '';
  return '=HYPERLINK("https://www.google.com/maps?q=' + la + ',' + lo +
    '","แผนที่")';
}

/** ★ rev.2: สำรองแท็บใด ๆ เป็นแท็บใหม่ "<prefix>_<วันที่>_<เวลา>" (ค่าเท่านั้น)
 *  ใช้กับ SYS_MASTER_IDX ก่อนเฟส 1b แตะ (BK_IDX_*) และแท็บอื่นตามต้องการ */
function clBackupTabByName_(sheetName, prefix) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var src = ss.getSheetByName(sheetName);
  if (!src) return null;
  var stamp = Utilities.formatDate(new Date(), Session.getScriptTimeZone(),
    'yyyyMMdd_HHmmss');
  var name = (prefix || 'BK_TAB_') + stamp;

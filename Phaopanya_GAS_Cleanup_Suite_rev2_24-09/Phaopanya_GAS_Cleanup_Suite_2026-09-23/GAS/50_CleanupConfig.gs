 ...
 * Task 45 · พอร์ตจาก scripts/master_cleanup/ (Python, Task 44)
 * rev.2 (Task 47-48) — รับข้อตรวจ Grok 3 ฉบับ + code review ตัวเอง:
 *   1) แยกเฟส 1 เป็น 1a (ล้างข้อความ ไม่แตะ key) / 1b (ย้าย key + ซ่อม IDX ท้ายสุด)
 *      — พิสูจน์จากโค้ด production จริง: ปุ่ม 1 ไม่เขียนทับ NAME/ADDR แถวเดิม
 *        (เขียนเฉพาะ POINTS/LAT/LNG/LAST_SEEN และ N/O เมื่อ geo แมตช์)
 *        ดังนั้น 1a เดี่ยว ๆ ปลอดภัย และ 1b ต้อง atomic กับแพตช์ cleanThai
 *   2) เฟส 1b คำนวณ key ด้วย makeKey() ของ production จริง (ไม่ใช่ต่อข้อความ)
 *      — แถวที่ ADDR_CLEAN ขึ้นต้นด้วย "เขต/แขวง..." จะถูก cleanAddr ตัดอีกชั้น
 *        การต่อข้อความตรง ๆ จะได้ key ต่างจากฝั่งปุ่ม 1 → เกิด duplicate (แก้รูเดิม)
 *   3) เฟส 2 เพิ่มธง NV_CONFLICT / KNN_THIN / KNN_FAR + โหมดนำร่อง 10 แถว
 *      + คอลัมน์ MAPS_URL ในแท็บรายงานทุกแท็บ (คลิกเทียบพิกัดกับ Google Maps)
 * ============================================================================
 ...
 *   2) สร้างไฟล์ใหม่ชื่อ 50_CleanupConfig.gs … 59_CleanupPhase1b.gs (10 ไฟล์)
 ...
 *
 * ลำดับใช้งาน rev.2 (เปลี่ยนจากฉบับแรก — ตามฉันทามติตรวจ 3 ฝ่าย):
 *   ลบคอลัมน์ทดสอบ AB-AD → เฟส 0 → เฟส 2 (dry → นำร่อง 10 → ทำจริง วนจนหมด)
 *   → แก้ SOURCE ต้นเหตุ (MD-0003/ed08ffce) → เฟส 1a → 1c → เฟส 3 → 4 → 5
 *   → เฟส 1b (วางแพตช์ cleanThai + ย้าย key + ซ่อม IDX วันเดียวกัน) → Self-Test
 *
 * หลักการเหล็ก (สืบทอดจาก Task 43-47):
 *   1) ห้าม rerun ปุ่ม 3/3b ทั้งชีต — แถวที่มี GEO_LAYER ถูก skip อยู่แล้ว
 *   2) เฟส 1a ไม่แตะ MATCH_KEY (ปลอดภัยเดี่ยว ๆ) / เฟส 1b บังคับวางแพตช์ cleanThai
 *      พร้อมกันในวันเดียวกัน — โค้ดตรวจให้เอง (dual placement)
 ...
 *   6) เฟส 2 ทำจริงเฉพาะแถวที่ผ่านธงทั้งหมด — ติดธง = คิวตรวจมือ ไม่ใช่ข้อผิดพลาด
 * ============================================================================
 */

var CLEANUP_VERSION = 'cleanup-r2 (2026-09-23)';
...
var CLEANUP_CFG = {
  ...
  IDX_SHEET: 'SYS_MASTER_IDX',    // ชื่อแท็บดัชนีของปุ่ม 2 (เฟส 1b ใช้)
  K: 15,                           // จำนวนเพื่อนบ้าน kNN (เทียบเท่า Python)
  MIN_VOTES: 3,                    // เสียงขั้นต่ำของการโหวต X/U (เฟส 2) และ W (เฟส 3)
  MIN_KNN_DOMINANCE: 10,           // ★ rev.2: เสียง O ต้อง ≥10/15 จึงทำจริงอัตโนมัติ (ต่ำกว่า = ธง KNN_THIN ตรวจมือ)
  MAX_MED_M: 3000,                 // ★ rev.2: มัธยฐานระยะเพื่อนบ้านเกิน 3 กม. = ธง KNN_FAR (พินโดดเดี่ยว ตรวจมือ)
  PILOT_ROWS: 10,                  // ★ rev.2: จำนวนแถวนำร่องของเฟส 2 (แถวแรกคือ MD-0003)
  FAR_KM: 5.0,                     // เพื่อนบ้านไกลสุดเกิน 5 กม. = ชนบท/ตรวจมือ (เฟส 3)

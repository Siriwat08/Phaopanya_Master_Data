...
function cleanupPhase0(doBackup) {
  ...
  // ---- ★ rev.2: สถิติ SYS_MASTER_IDX (baseline ก่อนเฟส 1b) ----
  var idxStat = { present: false, rows: 0, ghosts: 0 };
  var idxSh = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CLEANUP_CFG.IDX_SHEET);
  if (idxSh && idxSh.getLastRow() >= 2) {
    idxStat.present = true;
    var idxData = idxSh.getRange(2, 1, idxSh.getLastRow() - 1, 1).getValues();
    idxStat.rows = idxData.length;
    for (var r = 0; r < idxData.length; r++) {
      var id = clStr_(idxData[r][0]).trim();
      if (id && !mdIds[id]) idxStat.ghosts++;
    }
  }
  ...
  var metrics = {
    ...
    yEn: n - yThai, baseDup: baseDup, idx: idxStat, backup: bk ? bk.name : null
  };
  ...
  var rows = [
    ...
    ['MATCH_KEY ซ้ำเดิม', baseDup + ' (บันทึกไว้เทียบหลังเฟส 1b)'],
    ['SYS_MASTER_IDX (ดัชนีปุ่ม 2)', idxStat.present ?
      idxStat.rows + ' แถว | ghost (MD_ID ไม่มีใน MASTER) ' + idxStat.ghosts +
      ' แถว — เฟส 1b จะซ่อมให้' : 'ไม่มีแท็บดัชนี — เฟส 1b จะสร้างให้']
  ];

...
function cleanupPhase3(apply) {
  ...
  for (var i = 0; i < n; i++) {
    ...
    rec.push(reason);
    rec.push(clMapsUrl_(row[c.LAT], row[c.LNG]));
  ...
  cleanupReportTab_('P3_FILL', ['ROW', 'MD_ID', 'V(now)', 'W(now)', 'X(now)', 'LAT', 'LNG',
    'W_VOTES', 'TOP_VOTED_AMPHOE', 'N_NEW_TH', 'O_NEW_TH', 'REASON', 'MAPS_URL'], fillRows);
  cleanupReportTab_('P3_REVIEW', ['ROW', 'MD_ID', 'V(now)', 'W(now)', 'X(now)', 'LAT', 'LNG',
    'W_VOTES', 'TOP_VOTED_AMPHOE', 'N_NEW_TH', 'O_NEW_TH', 'REASON', 'MAPS_URL'], reviewRows);
